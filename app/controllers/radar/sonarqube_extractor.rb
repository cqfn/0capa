...
class SonarqubeExtractor < MetricsBaseController
  ...
  def runAnalysis
    ...
    TomPushInfo.where(status: 'D').each do |pushInfo|
      ...
      response = HTTP["Content-Type": 'JSON', "Content-Language": 'en-US',
                      "Authorization": "Basic #{@extractor_seetings.apisecret}"].get(
                        url
                      )
  ...
  def _prepare_java_repo(repo_path, repo_name)
    ...
    if File.exist?("#{repo_path}/pom.xml")
      ...
      dependencies_set.each do |dep|
        puts dep.content
        next unless dep.content.downcase == 'sonar-maven-plugin'
    ...
    elsif File.exist?("#{repo_path}/build.gradle")
