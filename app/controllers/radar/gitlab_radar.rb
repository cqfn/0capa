...
class GitlabRadar < RadarBaseController
  ...
  def check_new_invitations()
    ...
    response = HTTP["PRIVATE-TOKEN": "#{settings.apisecret}"].get(
      ...
      repos.each {
        ...
        if (!TomProject.exists?(repoid: repo["id"], source: SOURCE))
          ...
          project = TomProject.new(
            ...
            invitation_id: repo["id"].to_s + "_-" + SOURCE,
  ...
  def get_commits_info(settings, repo_info)
    ...
    request_url = request_url.sub! "#start_date", (repo_info.last_scanner_date.nil? ? repo_info.repo_created_at : repo_info.last_scanner_date).to_s
    page_counter = 0
    while true
      page_counter += 1
      ...
      response = HTTP["PRIVATE-TOKEN": "#{getNextToken()}"].get(
      ...
      if response.code == 200
        ...
        if repo_info.last_commit_id != newest_commit["short_id"]
          ...
          commits.each do |commit|
            if (!TomCommitsMetric.exists?(repoid: repo_info.repoid, base_commit_id: commit["short_id"]))
      ...
      if response.code == 200
        ...
        issues_info.each do |issue|
          ...
          while true
            ...
            response_comments = HTTP["PRIVATE-TOKEN": "#{getNextToken()}"].get(
              issue["_links"]["notes"] + "?per_page=100&page=" + page_comments_counter.to_s, json: {},
            )
      ...
      if response.code == 200
        ...
        pulls_info.each do |pull|
          if (!TomPullInfo.exists?(pull_id: pull["id"]))
            ...
            response = HTTP["PRIVATE-TOKEN": "#{getNextToken()}"].get(
              request_url + "/" + pull["iid"].to_s + "/notes", json: {},
            )
  ...
  def get_workflows_info(settings, repo_info)
    ...
    while true
      ...
      if response.code == 200
        ...
        if workflows_info.length == 0
          break
        end

        workflows_info.each do |w|
          response = HTTP["PRIVATE-TOKEN": "#{getNextToken()}"].get(
            request_url + "/" + w["id"].to_s, json: {},
          )

          if response.code == 200
            workflows_info_det = JSON.parse(response)

            if (!TomWorkflowInfo.exists?(action_id: workflows_info_det["id"].to_s + "_" + repo_info.repoid))
              newRow = TomWorkflowInfo.new(
                repoid: repo_info.repoid,
                repo_fullname: repo_info.repo_fullname,
                action_id: workflows_info_det["id"].to_s + +"_" + repo_info.repoid,
                name: "gilab_release_" + workflows_info_det["iid"].to_s,
              ...
              wf = TomWorkflowInfo.where(action_id: workflows_info_det["id"].to_s + "_" + repo_info.repoid).first
              wf.name = "gilab_release_" + workflows_info_det["iid"].to_s
