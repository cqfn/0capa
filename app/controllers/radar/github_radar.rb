...
class GithubRadar < RadarBaseController
  ...
  def get_repos_from_query(url_query)
    ...
    while true
      ...
      puts "getting page repos -> " + page_counter.to_s
      ...
      if response.code == 200
        ...
        puts "repos count -> " + repos_info["items"].length.to_s
        if repos_info["items"].length == 0
          break
        end
  ...
  def getQueueCounter()
    ...
    puts "project_count -> " + project_count.to_s
      ...
      if response.code == 200
        ...
        if repo_info.last_commit_id != newest_commit["sha"]
          ...
          commits.each do |commit|
            if (!TomCommitsMetric.exists?(repoid: repo_info.repoid, base_commit_id: commit["sha"][0..6]))
